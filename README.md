...
| 📌 学习记录 | 点喇叭旁图钉把词记入当日列表（含释义快照与出处句）。**完整例句整句记入**（`Leaves change colour in autumn.` 就是它自己，释义取紧随其后的中文译文行，不再被切成 `colour in` 这种词典碎片；句子不进字母填空考试，但照常进复习排期）；词组行/词条行按三级判定：① 命中的**词典短语整批记入**（最长优先、互不重叠，上限 6 词，`[网络]` 抓来的专名与自由组合不算命中）；② 讲义词表行里词典没收的教材搭配（`social pressure 社会压力`）**整行成词**，释义就地取同行中文注释；③ 都没有才退化成一个单词 |
| 🔁 记忆曲线 | 钉住的词自动排进艾宾浩斯间隔 **1 / 2 / 4 / 7 / 15 / 30 天**，走完六档后每 60 天一次；「每日复习」页签列出当日到期词与后续排期日历，打开列表即算过了一轮（同日重复打开不加速） |
| ⏱ 番茄钟 | 25/5 专注循环 + 近 14 天统计柱状图；**到点弹庆祝卡**（今日番茄数、🍅 打卡排、下一阶段）；电脑休眠自动暂停，不把睡过去的时间算成专注 |
| 📅 专注日历 | 月视图按天涂热度（1 / 2 / 3–4 / 5+ 四档红），完成一个番茄就在当天 +1，今天带琥珀描边；可翻月，底部汇总本月番茄数、专注分钟与「坐下来过」的天数 |
| ✍️ 拼写考试 | 随机挖掉单词里 n 个字母回填（1–6 可调），对绿错红露正解给分；优先考当日到期复习词，**答错的词在曲线上退一档、次日再见** |
| 🔔 复习弹窗 | 右下角"伪广告"小卡：优先抽当日到期复习词，其次今日 📌 记录，都没有退回**内嵌词典**（ECDICT）随机词；无焦点不打断输入 |
| 🔄 在线升级 | 应用内检查本仓库 GitHub Releases，发现新版只提示一行、不弹窗打扰，一键前往下载（按平台自动挑安装包） |
| 🪟 主窗行为 | 启动即最大化；五个页面常驻可视树，切页只切显示——**回到讲义页还在原来那一行**，缩放/字号档位也不跳回默认 |
...
# 把界面离屏渲染成图片，末位可指定临时库文件造数据
# 第 4 位为渲染目标：studylog（默认，页签 0=每日学习 / 1=每日复习）· pomodoro（页签 0=计时 / 1=统计 / 2=专注日历）· celebrate（到点庆祝卡，两种配色都出图）
# 远程或无辅助功能权限的环境里点不动界面，就用这个直接看像素
dotnet src/Parrot.App/bin/Debug/net10.0/Parrot.App.dll --shot /tmp/ui.png 1 [库文件路径] [页面]
```

> 庆祝卡固定 **1×** 出图：无边框透明窗画进 192dpi 的 `RenderTargetBitmap` 时只有文字会翻倍（布局是对的），
> 很容易被误判成排版 bug——真机上卡片是正常的。

`--ocr` 会报告引擎是否可用、识别行数与耗时、正文门槛是否通过、生成的热点个数；热点后面还会标出
📌 的判定结果（`[整句]` = 点它会整句记入，`[释义]` = 已挂上的中文译文行）——反馈 📌 记错时请连这两项一起贴。
喇叭没出现时先看这里的输出定位。
...
首次加载的多页识别是**并发**的（按 CPU 核数取 2–6 路滑动窗口，识别完成的先后即进度顺序）：
实测 46 页扫描件讲义首次标注 🔊 从 45.8 秒降到 13.7 秒，热点数与串行一字不差。
页渲染仍在 PDFium 全局锁里串行（库非线程安全），所以并发省下的是识别那一大段。

识别状态条是**会自己走的**：进度「OCR 标注原图🔊 n/N…」跑完换成完成提示，停留约 6 秒自动收起；
一页也没标出句子（整册图形页）或全部页命中缓存时，"正在后台识别"那句进行态说明同样被撤掉，不留常驻假状态。
...
```
src/
├─ Parrot.Core     # 领域逻辑：版式重建/断句/热点锚点/番茄钟状态机/艾宾浩斯档位/升级解析（不依赖 UI）
├─ Parrot.Audio    # 发音链路：Edge TTS/有道/系统音色 + 磁盘缓存 + 降级
├─ Parrot.Data     # SQLite：词典/学习记录/复习进度（review_state）/番茄统计/设置（Meta KV）
...
└─ Parrot.App      # 组合根：DI/托盘/老板键/弹窗/到点庆祝卡/自动升级/CLI 自检
...
- 记忆曲线上线之前钉住的老词，首次启动时按其**最早钉住日**回填排期（早过期的直接落到当日复习队列）；
  回填幂等、只补缺失词，已有档位与答错记录一律保留；
